...
class GaussianBaseModel(BaseGeometry, GaussianIO):
    ...
    def training_setup(self):
        ...
        #     {
        #         "params": [self._segment_p],
        #         "lr": self.cfg.segment_feature_lr,
        #         "name": "segment_p",
        #     },
        # ]
        if self.cfg.pred_normal:
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "segment_p":
                param_group["lr"] = self.cfg.segment_feature_lr
    ...
    def to(self, device="cpu"):
        ...
        self._segment_p = self._segment_p.to(device)
    ...
    def densify_double(self, N=1):
        stds = self.get_scaling.repeat(N, 1) / N
        means = torch.zeros((stds.size(0), 3), device="cuda")
        samples = torch.normal(mean=means, std=stds)
        rots = build_rotation(self._rotation).repeat(N, 1, 1)
        new_xyz = torch.bmm(rots, samples.unsqueeze(-1)).squeeze(-1) + self._xyz.repeat(N, 1)
        new_segment_p = self._segment_p.repeat(N, 1)
        new_scaling = self.scaling_inverse_activation(self.get_scaling.repeat(N, 1) / (0.8 * N))
        new_rotation = self._rotation.repeat(N, 1)
        new_features_dc = self._features_dc.repeat(N, 1, 1)
        new_features_rest = self._features_rest.repeat(N, 1, 1)
        new_opacity = self._opacity.repeat(N, 1)
        if self.cfg.pred_normal:
            new_normal = self._normal.repeat(N, 1)
        else:
            new_normal = None
